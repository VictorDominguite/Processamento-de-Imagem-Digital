...
def get_pixel_value(padded_img, i, j, n):
    section = padded_img[i-(n//2):i+(n//2), j-(n//2):j+(n//2)].astype(np.uint32)
    pixel = int(padded_img[i, j])
    if abs(pixel - np.max(section)) > abs(pixel - np.min(section)):
        return 0
    else:
        return 1

def metodo_contraste(img, n):
    ...
    for i in range(n, img.shape[0] + n):
        for j in range(n, img.shape[1] + n):
            result[r, c] = get_pixel_value(padded_img, i, j, n)
            c += 1
        c = 0
        r += 1

    cv2.imshow("Metodo do Contraste", result)
    cv2.waitKey(0)
    return result
...
result = metodo_contraste(img, 15)
