...
def get_pixel_value(padded_img, i, j, n, C):
...
def metodo_media(img, n, C):
    ...
    for i in range(n, img.shape[0] + n):
        for j in range(n, img.shape[1] + n):
            result[r, c] = get_pixel_value(padded_img, i, j, n, C)
            c += 1
        c = 0
        r += 1

    cv2.imshow("Metodo da Media", result)
    cv2.waitKey(0)
    return result
